...
struct EXYNOS4412_GPIO{
	...
	unsigned long gpf0_gpf1_reserve[2];
	...
	unsigned long gpf1_gpf2_reserve[2];
	...
	unsigned long gpf2_gpf3_reserve[2];
...
struct EXYNOS4412_LCD_REGISTER{
	...
	unsigned long sha_win_reserve;
	unsigned long winchmap2;

	unsigned long vidosd0a;
	unsigned long vidosd0b;
	unsigned long vidosd0c;
	unsigned long v0c_1a_reserve;

	unsigned long vidosd1a;
	unsigned long vidosd1b;
	unsigned long vidosd1c;
	unsigned long vid0sd1d;

	unsigned long vidosd2a;
	unsigned long vidosd2b;
	unsigned long vidosd2c;
	unsigned long vid0sd2d;


	unsigned long vidosd3a;
	unsigned long vidosd3b;
	unsigned long vidosd3c;
	unsigned long v3c_v4a_reserve;

	unsigned long vidosd4a;
	unsigned long vidosd4b;
	unsigned long vidosd4c;
	unsigned long v4c_reserve;

	unsigned long v4c_vw0_reserve[4];
	
	unsigned long vindw00add0b0;	/*VIDWxxADDx: Specifies source image address setting.*/
	unsigned long vindw00add0b1;	//		unsigned long vindw00add0b2;
	unsigned long vindw01add0b0;
	unsigned long vindw01add0b1;	//		unsigned long vindw01add0b2;

	unsigned long vindw02add0b0;
	unsigned long vindw02add0b1;	//		unsigned long vindw02add0b2;
	unsigned long vindw03add0b0;
	unsigned long vindw03add0b1;	//		unsigned long vindw03add0b2;

	unsigned long vindw04add0b0;
	unsigned long vindw04add0b1;	//		unsigned long vindw04add0b2;
	unsigned long vadd0_add1_reserve[2];
	
	unsigned long vindw00add1b0;
	unsigned long vindw00add1b1;	//		unsigned long vindw00add1b2;
	unsigned long vindw01add1b0;
	unsigned long vindw01add1b1;	//		unsigned long vindw01add1b2;		

	unsigned long vindw02add1b0;
	unsigned long vindw02add1b1;	//		unsigned long vindw02add1b2;
	unsigned long vindw03add1b0;
	unsigned long vindw03add1b1;	//		unsigned long vindw03add1b2;

	unsigned long vindw04add1b0;
	unsigned long vindw04add1b1;	//		unsigned long vindw04add1b2;
	unsigned long vadd1_add2_reserve[2];
...
static volatile unsigned long* clk_src_lcd0;
static volatile unsigned long* clk_drv_lcd;
static volatile unsigned long* gpd0_con;
static volatile unsigned long* gpd0_dat;
...
static int exynos4412_lcd_init(void){
	...
	lcdblk_cfg->lcdblk_cfg |= (0x1<<1);
	lcdblk_cfg->lcdblk_cfg2 |= (0x1<<0);
	...
	/*
	 * LCD的PWM使能输出 (直接作为一个out的管脚，省去了pwm驱动lcd设备背光亮度的麻烦)
	...
	gpd0_dat = gpd0_con + 1;
	writel((readl(gpd0_con) & ~(0xf<<4)) | (0x1<<4), gpd0_con);
	writel(readl(gpd0_dat) | (0x1<<1), gpd0_dat);
	 ...
	 *	CLKVAL_F	bit[13:6] VCLK = SCLK_FIMD/(CLKVAL+1), where CLKVAL>=1(VCLK <= 80MHZ)
	 *						 		51.2 = 100/(CLKVAL + 1) -> CLKVAL == 1
	 * 
	 ...
	 * VSPW	bit[7:0]	9 = 垂直同步脉冲宽度(行为单位)
	 */
	fb_reg->vidtcon0 = (22<<16)|(12<<8)|(10<<0);
	/*
	 * VIDTCON1 	配置视频输出时钟和决定显示尺寸
	 * HBPD	bit[23:16]	160 = 水平后肩 = 垂直同步信号结束到新的一帧数据开始传递需要多少vclk
	 * HFPD bit[15:8]	150 = 水平前肩 = 一帧数据结束到垂直同步信号的开始需要多少vclk
	 * HSPW	bit[7:0]	10 = 水平同步脉冲宽度(VCLK为单位)
	 */
	fb_reg->vidtcon1 = (160<<16)|(150<<8)|(10<<0);
	 ...
	 *
	 *	VIDOSD0B
	 *	OSD_RightTopX_F  [21:11]	Specifies the horizontal screen coordinate for right top pixel of OSD image.
	 *	OSD_RightTopY_F  [10:0]	Specifies the vertical screen coordinate for tight top pixel of OSD image.
	 *
	 *	VIDOSD0C
	 *	OSDSIZE  [23:0]	Specifies the Window Size For example, Height*Width (number of word)
	 */
	fb_reg->vidosd0b = (1023<<11)|(599<<0);
