...
#define SMPLRT_DIV		0x19	//采样分频寄存器地址（编号25），采样频率= 陀螺仪输出频率/（1+SMPLRT_DIV）
#define CONFIG			0x1A 	//配置寄存器地址（编号26），功能：配置外部帧同步和低通滤波
#define GYRO_CONFIG		0x1B	//陀螺仪配置寄存器地址（编号27）， 
#define ACCEL_CONFIG	0x1C	//加速度配置寄存器地址（编号28）
#define ACCEL_XOUT_H	0x3B	//加速度计测量值寄存器地址（编号59～64）
#define ACCEL_XOUT_L	0x3C
#define ACCEL_YOUT_H	0x3D
#define ACCEL_YOUT_L	0x3E
#define ACCEL_ZOUT_H	0x3F
#define ACCEL_ZOUT_L	0x40
#define TEMP_OUT_H		0x41	//温度测量值寄存器地址（编号65 - 66）
#define TEMP_OUT_L		0x42
#define GYRO_XOUT_H		0x43	//陀螺仪测量值寄存器地址（编号67 ～ 72）
#define GYRO_XOUT_L		0x44
#define GYRO_YOUT_H		0x45
#define GYRO_YOUT_L		0x46
#define GYRO_ZOUT_H		0x47
#define GYRO_ZOUT_L		0x48
#define PWR_MGMT_1		0x6B	//电源管理寄存器地址（编号107）
...
static int mpu6050_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = register_chrdev_region(devno, 1, "fs4412_mpu6050");
	if (ret < 0) {
	...
	mpu6050_write_byte(client, PWR_MGMT_1, 0x08);
	mpu6050_write_byte(client, SMPLRT_DIV, 0x07);
	mpu6050_write_byte(client, CONFIG, 0x06);
	mpu6050_write_byte(client, GYRO_CONFIG, 0x0f8);
	mpu6050_write_byte(client, ACCEL_CONFIG, 0x0f8);
...
static int mpu6050_platform_probe(struct platform_device* p){

	printk("mpu6050_platform_probe success\n");
	return 0;
}

static int mpu6050_platform_remove(struct platform_device* p){

	printk("mpu6050_platform_remove done\n");
	return 0;
}
/*
...
static struct of_device_id mpu6050_dt_match[] = {
	...
	{.compatible = "mpu6050-2" },
	{.compatible = "mpu6050-3" },

	{/*northing to be done*/},
};
MODULE_DEVICE_TABLE(of, mpu6050_dt_match);

struct platform_driver platform_driver = {
	//struct device_driver driver;
	.driver = {
		.owner = THIS_MODULE,
		.name = "mpu6050_device_driver",
		.of_match_table = of_match_ptr(mpu6050_dt_match),
	},
	.probe 		= mpu6050_platform_probe,
	.remove 	= mpu6050_platform_remove,
};
...
//	.id_table 	= mpu6050_id, //平台设备匹配使用
};
static int mpu6050_init(void){
	platform_driver_register(&platform_driver);
	i2c_add_driver(&mpu6050_driver); 
	printk("mpu6050_init ok\n");
	return 0;
}
static void mpu6050_exit(void){
	i2c_del_driver(&mpu6050_driver);
	platform_driver_unregister(&platform_driver);
	printk("mpu6050_exit done\n");
}
